...
## 11. 백테스트 제거 및 가상 예수금 격리성 강화 [2026년 8월 2일 일요일]

### 11.1. 백테스트 관련 사항 전면 제거
- **백테스트 모듈 삭제**: 백테스트 시뮬레이션 및 가상 브로커가 정의되어 있던 `core/backtest.py` 파일을 프로젝트에서 안전하게 영구 제거하였습니다.
- **대시보드 UI 리팩토링**:
  - `dashboard.py` 상단 임포트문에서 `run_backtest`를 제거했습니다.
  - 사이드바 설정 화면에서 실행 모드 선택("실전 투자", "백테스트") 라디오 버튼을 제거하고 `"실전 투자"`로 강제 고정시켰습니다.
  - 데이터 수집 기간, 수수료, 세금 입력 필드 등 백테스트용 설정 입력 폼들을 삭제했습니다.
  - 대시보드 메인 뷰에서 백테스트 시뮬레이션을 돌리고 결과를 Plotly 그래프와 텍스트로 요약해 시각화하던 화면 영역(약 150라인 분량)을 완전히 제거하여 대시보드 구동 속도를 높였습니다.
  - 전략 저장 및 업데이트 버튼의 핸들러에서 불필요한 `if mode == "실전 투자"` 조건 분기문을 간소화했습니다.

### 11.2. 1인 1계좌 대응을 위한 전략별 가상 예수금 및 잔고 격리 보강
- **체결 동기화 시 가상 예수금(pool) 계산 정교화**:
  - `core/database.py` 내 `sync_trade_history_db` 함수에서 신규 체결 건 동기화 시 **CA 전략**에 대해서도 가 가상 예수금(`pool`)을 가감하도록 갱신 로직을 추가했습니다. (BUY일 때 `pool`에서 매수 대금 차감, SELL일 때 `pool`에 정산 금액 가산)
  - **CA 및 VR 전략** 모두 가상 예수금을 갱신할 때, 단순 거래 원금(`price * qty`) 대신 수수료 및 매도 세금이 정확히 차감된 최종 정산금액(`total_amount`)을 사용하도록 보강하여 오차 누적을 차단했습니다.
- **전략 엔진 가상 예수금 사용 및 오염 방지**:
  - `core/cavr.py` 내 `ValueRebalancingEngine.run_cycle`에서 실제 전체 계좌 예수금 API 조회(`self.broker.get_cash_pool()`) 대신 해당 전략 전용으로 격리된 가상 예수금(`self.state.pool`)을 기준으로 평가금액(E) 및 지정가 매수/매도 주문 호가를 계산하도록 수정했습니다.
  - 매 사이클 끝에 가상 `self.state.pool` 값을 실제 계좌의 전체 예수금으로 강제 덮어쓰던 동기화 코드를 완전히 제거하여 개별 전략 예산이 오염되지 않도록 격리성을 영구 확보했습니다.
- **스케줄러 모니터링 경고 격리**:
  - `core/scheduler.py` 내 `run_ca_strategies` 및 `run_vr_strategies`에서 주문 제출 직전 예수금 부족 메시지를 전송할 때, 실제 전체 계좌 예수금 기준이 아닌 전략별로 격리 관리되는 가상 예수금(`pool`)을 확인해 경고하도록 필터링 기준을 변경했습니다.
...
## 14. 무한매수법(CA) LOC 주문 호가 절사 오차 및 대시보드 조건표 정합성 보완 [2026년 9월 16일 수요일]

### 14.1. 부동소수점 절사 오차로 인한 매도 주문 가격 왜곡 버그 해결
- **문제점:**
  - 미국 주식 LOC Star% 매도 주문 시 이론가($70.0769)를 호가 단위로 셋째 자리 절사(버림)할 때, 이미 1차 절사된 $70.07에 대해 `_sell()` 내부에서 호가 보정(`adjust_price_by_tick`)이 중복 호출되면서 파이썬 부동소수점 오차(`70.07 * 100 = 7006.999999999999`)로 인해 `math.floor`가 `7006`을 반환, 실제 주문이 **$70.06**으로 부당하게 1센트 깎여 나가는 현상이 발생했습니다.
- **수정 ([core/brokers/toss.py](file:///d:/Python_D/tossinvest/core/brokers/toss.py), [core/brokers/kis_kr.py](file:///d:/Python_D/tossinvest/core/brokers/kis_kr.py)):**
  - `adjust_price_by_tick` 내에서 부동소수점 곱셈/나눗셈 시 미세 오차를 흡수할 수 있도록 `round(price * 100, 6)`, `round(price * 10000, 6)`, `round(price / tick, 6)` 스케일링 보정을 적용하여 정확히 `$70.07`이 보존되도록 수정했습니다.

### 14.2. LOC Star% 자전거래 방지 오프셋 상쇄 및 가격 역전 방지
- **문제점:**
  - Star% 이론가($70.0769) 기준 매도 가격은 절사(floor $\rightarrow$ $70.07)되고, 매수 가격은 자전거래 방지 오프셋(-$0.01) 차감 후 올림(ceil $\rightarrow$ $70.07)되어 오프셋 효과가 상쇄되거나, 매도 주문 버그와 결합 시 매수가격($70.07)이 매도가격($70.06)보다 높아지는 가격 역전이 발생했습니다.
- **수정 ([core/cavr.py](file:///d:/Python_D/tossinvest/core/cavr.py)):**
  - 큰수 LOC 매수 가격 산출 시, 확정된 매도 호가 대비 최소 1틱(US $0.01, KR 최소 호가단위) 반드시 낮게 들어가도록 상한 제약(`max_buy_allowed`)을 적용했습니다:
